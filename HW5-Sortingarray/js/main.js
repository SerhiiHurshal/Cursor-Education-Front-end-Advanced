const getRandomArray = (length, min, max) => {
  const randomArray = []
  for (i = 0; i < length; i++) {
    randomArray.push(~~(Math.random() * (max - min) + min))
  }
...
const getMedian = (...numbers) => {
  ...
  const halfOfLength = numbers.length / 2

  if (numbers.length & 1) {
    return numbers[Math.round(halfOfLength)]
  } else {
    return (numbers[halfOfLength] + numbers[halfOfLength + 1]) / 2
  }
}
