...
const createNewStudent = (() => {
    ...
    if(isBudgetInput.checked){
        student = new BudgetStudent(universityInput.value, courseInput.value, fullnameInput.value)
    } else{
        student = new Student(universityInput.value, courseInput.value, fullnameInput.value)
    }
...
const addStudent = (() =>{ 
    ...
    td[0].firstElementChild.src = `img/00${Math.floor(Math.random() * (7-1) +1)}.svg`
    ...
    td[6].textContent = `Is on budget: ${(student instanceof BudgetStudent)?"yes":"no"}`
...
const addMark = function(event){
    ...
    const averageMarkOfStudentEl = document.getElementById(`averageMarkStudent${id}`)
...
const dismissOrRecoverStudent = function(event){
    ...
    const marksOfStudentEl = document.getElementById(`marksOfStudent${id}`)
    const averageMarkOfStudentEl = document.getElementById(`averageMarkStudent${id}`)
    const isDismissedStudentEl = document.getElementById(`isDismissedStudent${id}`)
